...
interface VendorCreditNoteFiltersProps {
  creditNoteNo: string
  vendorName: string
  status: string
  dateFrom: string
...
export function VendorCreditNoteFilters({
  creditNoteNo,
  vendorName,
  status,
  dateFrom,
  dateTo,
}: VendorCreditNoteFiltersProps) {
  ...
  const push = useCallback(
    (params: {
      ...
      router.push(`/invoice/credit-notes/vendor?${sp.toString()}`)
    },
    [creditNoteNo, vendorName, status, dateFrom, dateTo, router],
  )
