...
import type { StandardRateLine } from '@/types/standard-rate'
import { DataTable } from '@/components/shared/DataTable'

const col = createColumnHelper<StandardRateLine>()
...
const columns = [
  ...
  col.display({
    ...
    size: 110,
    cell: (info) => (
      <span className="text-sm text-gray-700">{info.row.original.paymentTo || '—'}</span>
    ),
  }),
  col.display({
    id: 'sellRate',
...
export function StandardRateLinesSection({ lines }: Props) {
  if (lines.length === 0) {
    return (
      <div className="flex items-center justify-center py-12 text-sm text-gray-400">
        No rate lines for this standard rate.
      </div>
    )
  }
