...
export function VendorInvoiceFilters({
  ...
  const push = useCallback(
    (params: {
      ...
      const vin = params.vendorInvoiceNo ?? vendorInvoiceNo
      ...
      if (vin) sp.set('vendorInvoiceNo', vin)
      if (st && st !== 'ALL') sp.set('status', st)
      if (df) sp.set('dateFrom', df)
      if (dt) sp.set('dateTo', dt)
      router.push(`/invoice/vendor?${sp.toString()}`)
    },
    [documentNo, customerName, vendorInvoiceNo, status, dateFrom, dateTo, router],
  )
  ...
  return (
    <>
      ...
      <SearchInput
        ...
        onChange={(v) => push({ vendorInvoiceNo: v })}
        placeholder="Search by vendor inv no…"
        className="w-52"
      />
